...
This directory also contains files consisting of just the `sentence_good` sentences in the BLiMP .jsonl files, separated by newlines. **These files should also NOT be committed to the repo.** Instead, in the `scripts/` subdirectory of this folder, you will find a script `blimp_json2corpus.sh` that will generate such files from the .jsonl files. It can be run in the following manner:
